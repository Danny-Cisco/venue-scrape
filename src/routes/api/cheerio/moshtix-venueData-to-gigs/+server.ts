...
async function fetchGigObjectsFromMoshtix(fetchFn: typeof fetch, urls: string[]) {
	const res = await fetchFn('/api/cheerio/moshtix-json2', {
		method: 'POST',
...
export async function POST({ request, fetch }) {
	...
	if (!Array.isArray(venues) || venues.length === 0) {
		return new Response(JSON.stringify({ error: 'Missing or invalid `venues` array in body' }), {
			status: 400,
			headers: { 'Content-Type': 'application/json' }
		});
	}
	...
	try {
		...
		for (const venue of venues) {
			const links = await extractMoshtixLinksFrom(venue.moshtix);
