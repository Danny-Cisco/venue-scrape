<script>
	...
	async function getDateRange(timeRangePrompt) {
		...
		const res = await fetch(`/api/supabase/get-filtered-date?table=soldout_gigs&${params}`);
	...
	onMount(async () => {
		try {
			...
			const dateRangeStartISO = sixMonthsAgo.toISOString();

			// 🕓 6 months ahead
			const sixMonthsAhead = new Date(now);
			sixMonthsAhead.setMonth(sixMonthsAhead.getMonth() + 6);
			const dateRangeEndISO = sixMonthsAhead.toISOString();
...
{#if pageInitialising}
	<div class="page center h-[100vhs]" in:fade>
		<p>Fetching soldout gigs from this year...</p>
		<PacMan />
	</div>
{:else if loading}
	<div class="page center h-[100vhs]" in:fade>
